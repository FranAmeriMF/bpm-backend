...
export class ReportesService {
  ...
  async getTramitesPorEstado(fecha_desde?: string, fecha_hasta?: string) {
    const where: any = {};
  ...
  async getDesempenoDecisor(jefe_id: string) {
    const decisiones = await this.prisma.decisionFinal.findMany({
      where: { jefe_decisor_id: jefe_id },
    });
    const total = decisiones.length;
    if (total === 0) return { total: 0, aprobados: 0, rechazados: 0, observados: 0, tasa_aprobacion: '0%' };
  ...
  async getReporteEmpresa(empresa_id: string) {
    ...
    const porEstado = tramites.reduce<Record<string, number>>((acc, t) => {
