...
const pool = new Pool({ connectionString: process.env.DATABASE_URL || 'postgresql://postgres:4dm1nDB@localhost:5433/db?schema=public' });
...
async function main() {
  ...
  // ─── Limpiar datos previos (orden respeta FK) ────────────────────────────
  ...
  // Asignar solicitante a la empresa y a su oficina
